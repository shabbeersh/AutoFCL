...
parser.add_argument("total_layers", help="No. of layers you want",
                    type=int)
...
RESULTS_PATH = f"results_caltech_{args.total_layers}.csv" # The path to the results file
...
def make_base_model(total_layers, include_top_ = True):
    x_in = layers.Input(shape=(224, 224, 3))
    x = x_in

    total = total_layers//3 + 1
    while total > 1:
    ...
    remaining_layers = total_layers % 3

    for j in range(remaining_layers):
        x = layers.Conv2D(filters=2, 
                        kernel_size=1, 
                        strides=(1, 1), 
                        padding='valid',
    ...
    if include_top_ == True:
        x = layers.Flatten()(x)
        # for i in range(total_layers//3 + 1):
...
# base_model = make_base_model(total_layers=3, include_top_=True)
...
# tuning the model
base_model = make_base_model(total_layers=args.total_layers, include_top_=True)
# base_model = models.Model(inputs=base_model.inputs, outputs=base_model.outputs)
