...
try:
	log_df = pd.read_csv(os.path.join("AutoFC_ResNet", "AutoFC_ResNet_log_random_search_Corel_10k.csv"), header=0, index_col=['index'])
...
for i in num_layers:
	...
	for j in temp_store:
		...
		new_model.compile(optimizer='adagrad', loss='categorical_crossentropy', metrics=["accuracy"])
		start = time.time()
		history = new_model.fit_generator(train_generator, steps_per_epoch=len(train_generator)/32, validation_data=valid_generator, validation_steps=len(valid_generator)/32, epochs=20)
	#print(f"Saving model {FILE_NAME}.")
	#new_model.save(FILE_PATH)

		time_taken = time.time() - start
		#print(new_model.evaluate_generator(valid_generator, verbose=1,steps=len(valid_generator)))
		print(new_model.metrics_names)
	...
	# log the reults in the log dataframe
		...
		loss, acc = new_model.evaluate_generator(train_generator,steps=len(valid_generator))
		val_loss, val_acc = new_model.evaluate_generator(valid_generator,steps=len(valid_generator))
...
#print(log_df.head())
		log_df.to_csv(os.path.join("AutoFC_ResNet", "AutoFC_ResNet_log_random_search_Corel_10k.csv"))
