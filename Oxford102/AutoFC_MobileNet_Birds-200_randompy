...
train_images = []
train_images_labels = []
TRAIN_PATH = os.path.join("Birds-200", "training")
VALID_PATH = os.path.join("Birds-200", "validation")
...
valid_datagen = image.ImageDataGenerator(preprocessing_function=keras.applications.mobilenet.preprocess_input)
...
try:
    log_df = pd.read_csv(os.path.join("AutoFC_MobileNet", "AutoFC_MobileNet_log_Birds-200_random_search_v1.csv"), header=0, index_col=['index'])
...
#print(log_df.head())
        log_df.to_csv(os.path.join("AutoFC_MobileNet", "AutoFC_MobileNet_log_Birds-200_random_search_v1.csv"))
