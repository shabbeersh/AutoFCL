...
RESULTS_PATH = os.path.join("AutoFC_DenseNet", "AutoFC_DenseNet_log_" + DATA_FOLDER + "_random_search_v1.csv") # The path to the results file
...
for i in num_layers:
    ...
    for j in temp_store: # iterate over the list of sampled configurations
        ...
        base_model = DenseNet121(weights="imagenet") # make an instance of an existing architecture to start the experiment
        for layer in base_model.layers: # freeze the existing layers to make them non-trainable
            layer.trainable = False

        X = base_model.layers[-2].output # removing the last fully ononected layers
